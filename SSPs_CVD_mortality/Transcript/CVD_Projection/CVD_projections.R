# =============================================================================
# CVD_PROJECTIONS -- SMOOTHED FRONTIER, SSP-ORDERED, NO BMI, WORLD BANK REGION
# Created: 11/09/2026
# Author: Martina Otavova
#
# Risk-factor input: Data/ihme_risk_factors_smooth_frontier_no_bmi.rds --
#   - "smoothed": frontier_p85 (the income-frontier penalty in the upstream
#     risk-factor pipeline) is interpolated continuously in gdp instead of
#     looked up from discrete GDP-quantile bins, removing the step-jump
#     artifact a country incurs when its gdp crosses a bin edge (found via
#     the Kiribati BMI investigation).
#   - "ordered": SSP-ordering enforcement (sort-and-reassign across the 5
#     scenarios so SSP3 is always most adverse, SSP5 least) is left ENABLED,
#     unchanged from production -- confirmed empirically to match the
#     upstream pipeline's `value_ordered` column exactly, not the
#     pre-ordering `pred_value_final`.
#   - "no BMI": BMI is blanked (all-NA) in the input file, so age_rfs
#     detection below drops it from every age group's model automatically.
#
# Model region random effect: World Bank regional classification (6
# geographic regions + a separate High-Income bucket, same scheme used for
# this project's regional reporting/plots) instead of UN subregion --
# see make_un_subregion() below, which is repointed to
# world_bank_classification despite keeping its original function name.
#
...
# produced here -- Transcript/Plots/CountryPlots.R covers all 188 countries
...
output_dir <- "/home/otavova/Healthy-SSPs/SSPs_CVD_mortality/Results/Projections/smooth_frontier_no_bmi_wbregion" # risk factors smoothed-frontier, SSP-ordered, BMI excluded, AND World Bank regional classification for the model's region random effect instead of UN subregion; nested under Results/Projections/ so production's own proj_spline.rds/obs_and_proj.rds at that top level are untouched
...
# TEST VARIANT: use the World Bank regional classification (the same one
# used for this project's regional reporting/plots -- 6 geographic regions
# plus a separate High-Income bucket) as the model's region/iso3 random-
# effects grouping, instead of UN subregion. Function name kept identical
# (make_un_subregion) so every call site below is unchanged.
source("Transcript/WorldBankClassification.R")  # adds world_bank_classification (iso3, region), covers all 204 project countries
wb_region_lookup <- setNames(world_bank_classification$region, world_bank_classification$iso3)

make_un_subregion <- function(iso3) {
  unname(wb_region_lookup[iso3])
}
...
risk_factors <- readRDS(file.path(input_dir, "ihme_risk_factors_smooth_frontier_no_bmi.rds")) %>%  # TEST VARIANT: smoothed-frontier risk factors, SSP-ordered (same as production), and BMI excluded entirely (all-NA column, so age_rfs detection drops it)
...
models_by_age <- obs_model_full %>%     # fit one mixed model per age group, since available risk factors differ by age
  ...
  group_modify(~ {                          # for each age group's slice (.x) and key (.y), return a one-row summary tibble
    ...
    age_rfs_z <- if (length(age_rfs) > 0) paste0(age_rfs, "_z") else character()  # TEST VARIANT: guard against the empty-vector paste0() pitfall (paste0(character(0), "_z") yields a stray "_z", not character(0)) -- can happen now that bmi is blanked and some young age groups had only bmi
...
proj_frame_base <- pop %>%                                    # start from interpolated population (annual, not just every 5 years)
  ...
  mutate(
    ...
    on_mx_grid = !is.na(mx_allcause),                                # TRUE only for years actually present in the 5-year mortality grid (before the zero-check below)
  ...
  group_modify(~ {                          # .x = this age's projection rows, .y = the age-group key
    ...
    proj_age %>%
      ...
      mutate(
        ...
        pred_allcause_deaths = mx_allcause * pop,                           # NA here whenever mx_allcause is NA (a literal 0 on-grid, or an off-grid year)
        pred_cardio_deaths = pred_ratio_cardio * pred_allcause_deaths,        # inherits that same NA when all-cause deaths are unknown
        pred_allcause_deaths = ifelse(on_mx_grid, coalesce(pred_allcause_deaths, 0), pred_allcause_deaths),  # on-grid + literal-0 mx means 0 deaths, not a missing row; off-grid years stay NA and get dropped below, unchanged from before
        pred_cardio_deaths = ifelse(on_mx_grid, coalesce(pred_cardio_deaths, 0), pred_cardio_deaths)
      ) %>%
...
proj_spline <- proj_spline[complete.cases(proj_spline),]   # drop rows still missing anything (e.g. an unresolved covariate) -- the mx_allcause-driven gap is now filled with 0 above, so it no longer causes rows to be dropped
...
models_by_age_g3 <- obs_model_full_g3 %>%
  ...
  group_modify(~ {
    ...
    age_rfs_z <- if (length(age_rfs) > 0) paste0(age_rfs, "_z") else character()  # TEST VARIANT: guard against the empty-vector paste0() pitfall
